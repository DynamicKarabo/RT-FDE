...
public sealed class RedisBehaviourStore : IBehaviourStore
{
    ...
    public async Task<BehaviouralContext?> GetBehaviourAsync(Guid userId, Guid transactionId, CancellationToken ct = default)
    {
        ...
        catch (RedisConnectionException ex)
        {
            _logger.LogWarning(
                ex,
                "Redis connection failure for user {UserId}. TransactionId: {TransactionId}. Proceeding with degraded scoring.",
                userId, transactionId);
            throw new BehaviourStoreUnavailableException("Redis connection unavailable.", ex);
        }
